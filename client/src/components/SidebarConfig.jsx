      ...
      {(appMode === 'new' || hasSession) && (
        <div className={styles.sidebarGroup}>
          ...
          <div className={styles.formGroup}>
            ...
            <div className={styles.selectWrapper}>
              <select
                value={config.negotiationMode || 'get_me_a_truck'}
                onChange={e => updateConfig("negotiationMode", e.target.value)}
                disabled={hasSession}
                style={{ width: '100%', appearance: 'none', paddingRight: '32px' }}
              >
                <option value="get_me_a_truck">Get me a truck</option>
                <option value="get_me_best_price">Get me best price</option>
              </select>
              <svg className={styles.selectIcon} width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"></polyline></svg>
            </div>
          </div>
